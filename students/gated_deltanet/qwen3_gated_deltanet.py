...
class Qwen3GatedDeltaNetConfig(Qwen3Config):
    model_type = "qwen3_gated_deltanet"
    ...
    def forward(
        ...
        **kwargs
        ...
        if attention_mask is not None:
            ...
            hidden_states = index_first_axis(rearrange(
                hidden_states, "b s ... -> (b s) ..."),
                indices
            ).unsqueeze(0)
...
class Qwen3GatedDeltaNetDecoderLayer(GradientCheckpointingLayer):
...
class Qwen3GatedDeltaNetModel(Qwen3PreTrainedModel):
    config_class = Qwen3GatedDeltaNetConfig
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [Qwen3GatedDeltaNetDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Qwen3GatedDeltaNetPreTrainedModel(Qwen3PreTrainedModel):
    config_class = Qwen3GatedDeltaNetConfig
...
class Qwen3GatedDeltaNetForCausalLM(Qwen3GatedDeltaNetPreTrainedModel, FLAGenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = Qwen3GatedDeltaNetModel(config)
        self.vocab_size = config.vocab_size
    ...
    def copy_from_teacher(self, teacher, copy_qkv=True):
        ...
        for self_layer, teacher_layer in zip(self.model.layers, teacher.model.layers):
            ...
            if copy_qkv:
                ...
                if self_layer.self_attn.q_norm.weight.shape == teacher_layer.self_attn.q_norm.weight.shape:
                    self_layer.self_attn.q_norm.load_state_dict(teacher_layer.self_attn.q_norm.state_dict())
                    self_layer.self_attn.k_norm.load_state_dict(teacher_layer.self_attn.k_norm.state_dict())
                else:
                    n_rep = self_layer.self_attn.head_qk_dim // teacher_layer.self_attn.head_dim
                    self_layer.self_attn.q_norm.weight.data.copy_(
                        teacher_layer.self_attn.q_norm.weight.data.repeat(n_rep)
                    )
                    self_layer.self_attn.k_norm.weight.data.copy_(
                        teacher_layer.self_attn.k_norm.weight.data.repeat(n_rep)
                    )
...
AutoConfig.register(Qwen3GatedDeltaNetConfig.model_type, Qwen3GatedDeltaNetConfig)
AutoModel.register(Qwen3GatedDeltaNetConfig, Qwen3GatedDeltaNetModel)
AutoModelForCausalLM.register(Qwen3GatedDeltaNetConfig, Qwen3GatedDeltaNetForCausalLM)

StudentConfig = Qwen3GatedDeltaNetConfig
StudentModel = Qwen3GatedDeltaNetForCausalLM
