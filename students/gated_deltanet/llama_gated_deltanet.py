...
class LlamaGatedDeltaNetConfig(GatedDeltaNetConfig):
    model_type = 'llama_gated_deltanet'

    def __init__(self, rms_norm_eps: float = 1e-6, mlp_bias: bool = False, **kwargs):
    ...
    def forward(
        ...
        **kwargs
        ...
        if diagnostics is not None:
            alpha = torch.exp(g).clamp(0.0, 1.0)
...
class GatedDeltaNetDecoderLayer(GradientCheckpointingLayer):
...
class GatedDeltaNetModel(llama.LlamaPreTrainedModel):
    config_class = LlamaGatedDeltaNetConfig
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [GatedDeltaNetDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class LlamaGatedDeltaNetPreTrainedModel(PreTrainedModel):
    config_class = LlamaGatedDeltaNetConfig
...
class GatedDeltaNetForCausalLM(LlamaGatedDeltaNetPreTrainedModel, FLAGenerationMixin):
...
class GatedDeltaNetForSequenceClassification(llama.LlamaForSequenceClassification):
    def __init__(self, config):
        super().__init__(config)
...
AutoConfig.register(LlamaGatedDeltaNetConfig.model_type, LlamaGatedDeltaNetConfig)
AutoModel.register(LlamaGatedDeltaNetConfig, GatedDeltaNetModel)
AutoModelForCausalLM.register(LlamaGatedDeltaNetConfig, GatedDeltaNetForCausalLM)

StudentConfig = LlamaGatedDeltaNetConfig
StudentModel = GatedDeltaNetForCausalLM
