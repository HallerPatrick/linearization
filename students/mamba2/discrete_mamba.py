...
class Mixer(nn.Module):
    ...
    def convolutional_forward(self, xBC, padded_len):
        x_t = xBC.transpose(1, 2).contiguous()
        use_fast = causal_conv1d_fn is not None and self.activation in [
            "silu",
            "swish",
            "identity",
        ]
        if not use_fast:
            xBC = self.act(
                self.conv1d(x_t)[..., :padded_len].transpose(1, 2)
            )
        else:
    ...
    def convolutional_step(self, xBC, conv_state):
        ...
        use_fast = causal_conv1d_update is not None
        if use_fast:
