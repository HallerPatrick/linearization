...
class DiscreteMamba2(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Parameters for initialization as requested by user
        self.A_log = nn.Parameter(torch.empty(self.n_v_heads, **factory_kwargs))
        self.D = nn.Parameter(torch.ones(self.n_v_heads, **factory_kwargs))
        self.dt_bias = nn.Parameter(torch.empty(self.n_v_heads, **factory_kwargs))
    ...
    def forward(self, u, inference_params=None, **kwargs):
        ...
        # Project input
        xBCzdt = self.in_proj(u)

        xBC, z, dt = torch.split(
            xBCzdt,
            [
        ...
        result = mamba_chunk_scan_combined(
            x=x / F.softplus(dt).to(x.dtype).unsqueeze(-1),
            dt=dt,
            dt_softplus=True,
    ...
    def step(self, u, state, **kwargs):
        ...
        xBC, conv_state = self.convolutional_step(xBC.contiguous(), state["conv"])
        ...
        zeros = torch.zeros((self.n_v_heads, self.headdim), device=dt.device).to(dtype=x.dtype)
        
        # A = -exp(A_log)
        A = -torch.exp(self.A_log).to(x.dtype)
        ones = torch.ones((self.n_v_heads, self.headdim, self.d_state), device=dt.device).to(
            dtype=x.dtype
        )
        # selective_state_update expects A of shape (n_heads, head_dim, d_state) or (n_heads, d_state)
        # If A is (n_heads,), we need to broadcast it.
        # Actually it's often (n_heads, d_state) or (n_heads, head_dim, d_state)
        # Here we have A per head.
        A_expanded = A[:, None, None] * ones # (n_heads, head_dim, d_state)
