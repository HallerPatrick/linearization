...
from models.modular_xlstm import mLSTMLayerConfig, mLSTMLayer, mLSTMIntraLayer, xLlamaConfig, xLSTMCache
...
class LlamaDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: xLlamaConfig, layer_idx: int):
        ...
        # else:
        self.self_attn = mLSTMIntraLayer(mLSTMLayerConfig.from_xllama_config(config), intra_layer_swa=True)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if isinstance(output, tuple):
            hidden_states, state, attn = output
        else:
...
class xLlamaForCausalLM(xLlamaPreTrainedModel, GenerationMixin):
    ...
    def copy_from_teacher(self, teacher, copy_qkv: bool = True):
        ...
        for self_layer, teacher_layer in zip(self.model.layers, teacher.model.layers):
            ...
            # mLSTM Layer
            if hasattr(self_layer.self_attn, "q"):
                if copy_qkv:
            ...
            # Attn
            if copy_qkv and hasattr(self_layer.self_attn, "q_proj"):
                self_layer.self_attn.load_state_dict(
                    teacher_layer.self_attn.state_dict()
                )
