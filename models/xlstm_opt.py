...
from .modular_xlstm import xOPTConfig

class OPTDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: xOPTConfig, layer_idx: Optional[int] = None):
        super().__init__()
        ...
        self.self_attn = mLSTMLayer(mLSTMLayerConfig.from_xopt_config(config))
...
class OPTDecoder(OPTPreTrainedModel):
    ...
    config: xOPTConfig
    def __init__(self, config: xOPTConfig):
...
class xOPTForCausalLM(OPTPreTrainedModel, GenerationMixin):
    config: xOPTConfig
    ...
    def copy_from_teacher(self, teacher: "xOPTForCausalLM", copy_qkv: bool = True):
        # Embeddings + LM head
    ...
    def generate(
        ...
        **kwargs,
        ...
        #     sequences=sequences,
        #     scores=None if collected_scores is None else tuple(collected_scores),
...
AutoConfig.register("xopt", xOPTConfig)
AutoModel.register(xOPTConfig, OPTDecoder)
AutoModelForCausalLM.register(xOPTConfig, xOPTForCausalLM)
